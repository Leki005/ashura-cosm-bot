...
async def _generate_recommendations(answers: dict, for_admin: bool = False, user_id: int = 0) -> str:
    ...
    if for_admin:
        prompt = (
            ...
            f"{data_text}\n\n"
            "МОИ УСЛУГИ (рекомендуй ТОЛЬКО из этого списка):\n"
            "• Ботулинотерапия (Ботокс) — 15 000₽\n"
            "• Увеличение губ (филлер) — 12 000₽\n"
            "• Контурная пластика скул — 18 000₽\n"
            "• Мезотерапия лица — 6 000₽\n"
            "• Пилинг Jessner — 4 500₽\n"
            "• Пилинг Mandelic 40% — 3 200₽\n"
            "• Пилинг Salicylic 30% — 3 000₽\n"
            "• PRX-T33 — 5 500₽\n"
            "• Чистка лица (механическая) — 3 500₽\n"
            "• Чистка лица (ультразвуковая) — 4 000₽\n"
            "• Комбинированная чистка — 5 000₽\n"
            "• Карбокситерапия — 3 500₽\n"
            "• Лазерная эпиляция — от 2 500₽\n"
            "• Консультация косметолога — 4 000₽\n"
            "• Уходовый комплекс «Сияние» — 7 000₽\n\n"
            "Составь план:\n"
            "1. Оценка кожи\n"
            "2. Рекомендуемые процедуры (обязательные / рекомендуемые / по желанию)\n"
            "3. Домашний уход (утро/вечер)\n"
            "4. Противопоказания\n"
            "5. План на 2-3 месяца\n"
            "6. Что уточнить на приёме\n\n"
            "Пиши ПРОСТЫМ текстом на русском языке. Без символов ─, *, **, ###, markdown. "
            "Используй эмодзи для структуры (📋, 💊, ⚠️). "
            "Коротко и по делу, как сообщение коллеге."
        )
    else:
        prompt = (
